...
from gear_sonic.data.features_sonic_vla_gr00t import (
...
class GrootDataCollector:
    ...
    def _add_sonic_pose_features(self, frame_data: dict) -> float | None:
        ...
        # SMPL features — Causal Alignment design:
        #   teleop.*  = raw VR input captured this step  (observation role)
        #   action.*  = same data, semantically labelled as the causal expert
        #               response to the robot observation at the START of this
        #               step — the training target for Imitation Learning.
        if use_smpl and smpl_msg.get("smpl_joints") is not None:
            joints = np.ascontiguousarray(
                np.asarray(smpl_msg["smpl_joints"], dtype=np.float32).flatten()
                if smpl_msg["smpl_joints"].ndim == 2
                else np.asarray(smpl_msg["smpl_joints"], dtype=np.float32)
            )
            frame_data["teleop.smpl_joints"] = joints
            frame_data["action.smpl_joints"] = joints
        else:
            frame_data["teleop.smpl_joints"] = np.zeros(72, dtype=np.float32)
            frame_data["action.smpl_joints"] = np.zeros(72, dtype=np.float32)
        ...
        if use_smpl and smpl_msg.get("body_quat_w") is not None:
            ...
            frame_data["action.body_quat_w"] = body_quat_w
            ...
            frame_data["teleop.body_quat_w"] = _zero_quat
            frame_data["action.body_quat_w"] = _zero_quat
            frame_data["teleop.target_body_orientation"] = quat_to_rot6d(_zero_quat)
        ...
        frame_data["teleop.left_hand_joints"] = _left_hand
        frame_data["action.left_hand_joints"] = _left_hand
        frame_data["teleop.right_hand_joints"] = _right_hand
        frame_data["action.right_hand_joints"] = _right_hand
