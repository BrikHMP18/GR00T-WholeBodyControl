"""
Dataset configuration for the Sonic VLA pipeline — GR00T variant.

Extends the base ``features_sonic_vla`` schema with explicit ``action.*``
keys for the four VR/teleop signals that serve as causal action labels
for Imitation Learning:

    action.smpl_joints        (72,)  float32
    action.body_quat_w         (4,)  float32
    action.left_hand_joints    (7,)  float32
    action.right_hand_joints   (7,)  float32

Causal Alignment design
-----------------------
  teleop.*  — raw VR input captured at the START of each control step.
               Acts as the observation: "this is what the operator was
               doing when the robot was in state X".
  action.*  — same numerical data, semantically re-labelled as the
               expert's causal response to the robot observation at that
               step.  This is the training target for the policy:
               Observation X  →  Action X.

The modality config ``action`` section therefore points to the
``action.*`` original_keys so that GR00T reads the correctly-labelled
...
def get_modality_config_sonic_vla(robot_model: RobotModel) -> dict:
    """Return the modality config for the Sonic VLA GR00T dataset.

    Produces the exact content of meta/modality.json.

    The ``action`` modality points to ``action.*`` original_keys so that
    GR00T reads the causally-aligned expert targets during training.
    """
    ...
    return {
        ...
        "action": {
            ...
            # --- Causally-aligned VR action labels (action.* keys) ---
            "smpl_joints": {
                "start": 0,
                "end": 72,
                "original_key": "action.smpl_joints",
            },
            "smpl_pose": {
                "start": 0,
                "end": 63,
            ...
            "body_quat_w": {
                "start": 0,
                "end": 4,
                "original_key": "action.body_quat_w",
                "rotation_type": "quaternion",
            },
...
def get_features_sonic_vla(robot_model: RobotModel) -> dict:
    """Return the dataset features for the Sonic VLA GR00T dataset.

    The returned dict populates the "features" key of meta/info.json.

    Four ``action.*`` keys are added alongside their ``teleop.*``
    counterparts, implementing the Causal Alignment design for
    Imitation Learning (see module docstring).
    """
    ...
    return {
        ...
        # --- teleop.* observation copies (raw VR input) ---
        "teleop.smpl_joints": {
            "dtype": "float32",
            "shape": (72,),
            "names": "smpl_joints",
        ...
        # --- action.* causal labels (same data, training targets) ---
        "action.smpl_joints": {
            "dtype": "float32",
            "shape": (72,),
            "names": "smpl_joints",
